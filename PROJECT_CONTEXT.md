...
Each active IB and IGCSE question card includes an optional temporary whiteboard for handwritten working. It opens beside the question on larger screens and below it on smaller screens, supports pen, colour, eraser, undo and clear controls, and is intentionally not saved between page loads. Students can switch the working surface between squared paper, which is the default, and blank paper without changing their work. Clear acts immediately without a confirmation dialog and is itself reversible with Undo. The working sheet is taller than its visible panel and scrolls internally so the question remains in place. Students can click any question diagram to copy it onto that question's working sheet and annotate it. Do not introduce account storage or cloud syncing for whiteboard work unless Rob requests it later.
