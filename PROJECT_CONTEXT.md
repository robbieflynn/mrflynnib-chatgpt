...
The original course-specific MailerLite workflows sent once when a subscriber first joined a course group. On 20 August 2026, those four workflows were paused and replaced by one active workflow, **Send all four syllabus checklists** (`196293930178839981`), triggered by joining the **Mr Flynn IB mailing list** group. The replacement workflow sends one email linking to all four PDFs and allows subscribers to re-enter. The website resets and restores that marketing-group membership on each valid request so the same address can request the bundle again. Keep the form rate limit in place to prevent abuse.

The checklist is the welcome resource for joining the Mr Flynn IB email list. As of 20 August 2026, every requester receives all four IB Mathematics syllabus checklists in one welcome email, while the selected course is still recorded for segmentation. The confirmation page also provides immediate links to all four PDFs so delivery does not depend entirely on email. The form uses one button whose label explicitly combines checklist delivery and subscription, supported by adjacent plain-language disclosure about occasional IB Mathematics resources and course updates and the ability to unsubscribe. Every completed request is added to both the relevant course group and the separate MailerLite group **Mr Flynn IB mailing list** (`195162439657981769`), with the opt-in time recorded. The group ID is configured in Vercel as `MAILERLITE_MARKETING_GROUP_ID` for Production and Preview. Future marketing campaigns should target this dedicated group rather than the checklist-delivery groups or all active contacts.
