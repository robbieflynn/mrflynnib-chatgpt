...
Each active IB and IGCSE question card includes an optional temporary whiteboard for handwritten working, including cards appended after filters or progressive loading. It opens beside the question on larger screens and below it on smaller screens, supports pen, six preset colours, eraser, undo and immediate undoable clear controls, and is intentionally not saved between page loads. The reversed end of a compatible pen such as a Microsoft Surface Pen erases temporarily without changing the selected toolbar tool. Students can switch the working surface between squared paper, which is the default, and blank paper without changing their work, zoom the sheet from 75% to 200%, and expand the board to the full question-card width. The working sheet is taller than its visible panel and scrolls internally so the question remains in place. Students can click any question diagram to copy it onto that question's working sheet and annotate it; imported diagrams live on a protected background layer and must not be erased by the ink eraser. Diagram hover highlighting and its instructional hint appear only while that question's whiteboard is open. Do not introduce account storage or cloud syncing for whiteboard work unless Rob requests it later.
